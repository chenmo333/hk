...
public class PlcService : IPlcService
{
    ...
    public int ReadDbInt(int db, int startByteAddress)
    {
        try
        {
            var bytes = _plc.ReadBytes(DataType.DataBlock, db, startByteAddress, 2);
            if (bytes.Length < 2)
            {
    ...
    public float ReadDbReal(int db, int startByteAddress)
    {
        try
        {
            var bytes = _plc.ReadBytes(DataType.DataBlock, db, startByteAddress, 4);
            if (bytes.Length < 4)
            {
    ...
    public bool ReadDbBool(int db, int startByteAddress, byte bitAdr)
    {
        try
        {
            var bytes = _plc.ReadBytes(DataType.DataBlock, db, startByteAddress, 1);
            if (bytes.Length < 1)
            {
    ...
    public string ReadDbString(int db, int startByteAddress, int maxLength)
    {
        try
        {
            var bytes = _plc.ReadBytes(DataType.DataBlock, db, startByteAddress, maxLength + 2);
    ...
    public void WriteDbString(int db, int startByteAddress, string value, int maxLength)
    {
        try
        {
            ...
            var bytes = new byte[maxLength + 2];
