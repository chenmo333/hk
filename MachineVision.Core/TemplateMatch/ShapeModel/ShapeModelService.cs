...
public class ShapeModelService : BindableBase, ITemplateMatchService
{
    public ShapeModelService()
    {
        Info = new MethodInfo()
        {
            ...
            {
                new() { Name = "Image", Description = "Input image in which the model should be found." },
                new() { Name = "ModelID", Description = "Handle of the model." },
                new() { Name = "AngleStart", Description = "Smallest rotation of the model." },
                new() { Name = "AngleExtent", Description = "Extent of the rotation angles." },
                new() { Name = "MinScore", Description = "Minimum score of the instances of the model to be found." },
                new()
                {
                    Name        = "NumMatches",
                ...
                new()
                {
                    Name = "MaxOverlap ", Description = "Maximum overlap of the instances of the model to be found."
                },
                new() { Name = "SubPixel", Description = "Subpixel accuracy if not equal to 'none'." },
                new()
                {
                    Name = "NumLevels",
                    Description =
                ...
                new() { Name = "Row", Description    = "Row coordinate of the found instances of the model." },
                new() { Name = "Column", Description = "Column coordinate of the found instances of the model." },
                new() { Name = "Angle", Description  = "Rotation angle of the found instances of the model." },
                new() { Name = "Score", Description  = "Score of the found instances of the model." }
            },
    ...
    private HTuple hv_Row   = new(), hv_Column = new();
    private HTuple hv_Angle = new(), hv_Score  = new();
        ...
        if (MatchResult.Results != null)
            foreach (var item in MatchResult.Results)
            {
                ...
                if (Setting.IsShowDisplayText)
                    HWindow.SetString($"({Math.Round(item.Row, 2)},{Math.Round(item.Column, 2)})", "image", item.Row,
                        item.Column, "black", "true");
